  document.documentElement.setAttribute("nimbus", "");
    ...
    `;
        ...
        gerarElementoInput(textoLabel, elementoPai, idInput, nameInput) {
          ...
          elementoInput.id = idInput;
          elementoInput.name = nameInput;

          const elementoLabel = document.createElement("label");
          elementoLabel.setAttribute("for", idInput);
          elementoLabel.textContent = textoLabel;

          containerInput.appendChild(elementoInput);
          containerInput.appendChild(elementoLabel);

          elementoPai.innerHTML = "";
          elementoPai.appendChild(containerInput);
        }
        ...
        applyStyles() {
          ...
          elements.forEach((element) => {
            ...
            const parts = nimbusValue.split(",").map((part) => part.trim());
            ...
            parts.forEach((part) => {
              if (part.startsWith("in{") && part.endsWith("}")) {
                labelText = part.slice(3, -1).trim();
              } else if (part.startsWith("id{") && part.endsWith("}")) {
                const ids = part
                  .slice(3, -1)
                  .split(" ")
                  .map((id) => id.trim());
            ...
            if (labelText && idInput) {
              this.gerarElementoInput(labelText, element, idInput, nameInput);
            }

            parts.forEach((part) => {
              if (part.startsWith("style-")) {
                const style = part.slice(6).trim();
        ...
        processStyle(style) {
          if (style.startsWith("background-")) {
            ...
            this.Nimbusmap[style] = [
              { property: "backgroundColor", value: cores[color] },
            ];
          } else if (style.startsWith("color-")) {
            const color = style.split("-")[1];
            this.Nimbusmap[style] = [
              { property: "color", value: cores[color] },
            ];
          } else if (style.startsWith("t-height-px-")) {
            const height = style.split("-")[3];
            this.Nimbusmap[style] = [
              { property: "height", value: `${parseInt(height)}px` },
            ];
          } else if (style.startsWith("t-height-cent-")) {
            const height = style.split("-")[3];
            this.Nimbusmap[style] = [
              { property: "height", value: `${parseInt(height)}%` },
            ];
          } else if (style.startsWith("t-width-cent-")) {
            const width = style.split("-")[3];
            this.Nimbusmap[style] = [
              { property: "width", value: `${parseInt(width)}%` },
            ];
          } else if (style.startsWith("t-width-px-")) {
            const width = style.split("-")[3];
            this.Nimbusmap[style] = [
              { property: "width", value: `${parseInt(width)}px` },
            ];
          } else if (style.startsWith("ts-px-")) {
            const size = style.split("-")[2];
            this.Nimbusmap[style] = [
              { property: "fontSize", value: `${parseInt(size)}px` },
            ];
          } else if (style.startsWith("ts-cent-")) {
            const size = style.split("-")[2];
            this.Nimbusmap[style] = [
              { property: "fontSize", value: `${parseInt(size)}%` },
            ];
            ...
            this.Nimbusmap[style] = [
              { property: "borderRadius", value: `${parseInt(radius)}px` },
            ];
          } else if (style.startsWith("borderad-cent-")) {
            const radius = style.split("-")[2];
            this.Nimbusmap[style] = [
              { property: "borderRadius", value: `${parseInt(radius)}%` },
            ];
          } else if (style.startsWith("gap-px-")) {
            const gap = style.split("-")[2];
            this.Nimbusmap[style] = [
              { property: "gap", value: `${parseInt(gap)}px` },
            ];
          } else if (style.startsWith("gap-cent-")) {
            const gap = style.split("-")[2];
            this.Nimbusmap[style] = [
              { property: "gap", value: `${parseInt(gap)}%` },
            ];
          } else if (style.startsWith("marg-px-")) {
            const margin = style.split("-")[2];
            this.Nimbusmap[style] = [
              { property: "margin", value: `${parseInt(margin)}px` },
            ];
          } else if (style.startsWith("marg-cent-")) {
            const margin = style.split("-")[2];
            this.Nimbusmap[style] = [
              { property: "margin", value: `${parseInt(margin)}%` },
            ];
            ...
            this.Nimbusmap[style] = [
              {
                ...
                value: `repeat(auto-fit, minmax(${minWidth}px, ${fraction}fr))`,
              },
            ];
          }
        }
